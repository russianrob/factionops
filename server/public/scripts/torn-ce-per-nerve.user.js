...
// @downloadURL  https://tornwar.com/scripts/torn-ce-per-nerve.user.js
// @updateURL    https://tornwar.com/scripts/torn-ce-per-nerve.meta.js
// ==/UserScript==
...
(function () {
    ...
    function serverFetch(url) {
        // fetch FIRST: tornwar.com serves CORS for www.torn.com, and warboard's
        // GM_xmlhttpRequest shim has been observed firing onerror for requests
        // the server actually completed. GM stays as the fallback for hosts
        // where fetch is blocked.
        if (typeof fetch === 'function') {
            const c = new AbortController();
            const t = setTimeout(() => { try { c.abort(); } catch (e) {} }, 10000);
            return fetch(url, { cache: 'no-store', signal: c.signal })
                .then(r => { clearTimeout(t); if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
                .catch(e => { clearTimeout(t); return serverFetchGM(url).catch(() => { throw e; }); });
        }
        return serverFetchGM(url);
    }

    function serverFetchGM(url) {
