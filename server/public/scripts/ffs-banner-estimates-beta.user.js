...
// @connect      tornwar.com
// @license      GPL-3.0
// @downloadURL  https://tornwar.com/scripts/ffs-banner-estimates-beta.user.js
// @updateURL    https://tornwar.com/scripts/ffs-banner-estimates-beta.meta.js
// ==/UserScript==
...
if (!singleton) {
  ...
  // beta: a target whose hospital time just ended is the most time-critical row
  ...
  // Factions already warmed this page load. The cache is a WARM START, not a
  // source of truth: it exists so countdowns are on screen before the first
  // poll lands, and once per faction is all that is for.
  const _ffsCacheHydrated = new Set();

  function ffs_loadStatusCache(factionId) {
    if (!factionId) return;
    // Once. pollAll calls this every cycle, and ffs_saveStatusCache writes the
    // WHOLE hospital map into localStorage, so re-loading assigned a snapshot
    // up to five minutes old straight over live data — then saved it again,
    // refreshing its own TTL, so a stale entry kept itself alive.
    //
    // Reported as: attack a target, their new hospital time appears, and within
    // thirty seconds the row drops back to 00:00:00 — the pre-attack snapshot.
    // The same clobber resurrects members the poll has already released, which
    // is how a freed target kept a timer frozen at zero.
    //
    // Marked before the read, so a faction with no cache is not retried on
    // every poll for the life of the page.
    if (_ffsCacheHydrated.has(factionId)) return;
    _ffsCacheHydrated.add(factionId);
    try {
  ...
  // and throttle so it survives to capture the moment that matters rather than
  // being spent at load, and a QUIET tag server-side so it gets its own bucket
  // instead of eating the shared 60/min every other script draws on. Fires only
  // while a chip is sitting at zero — which is exactly the state being
  // diagnosed. Remove once Torn's wording for a released member is known.
  let _ffsHospDiagN = 0, _ffsHospDiagAt = 0;
  function ffs_hospDiag(payload) {
    if (_ffsHospDiagN > 12) return;
    const now = Date.now();
    if (now - _ffsHospDiagAt < 3000) return;
    _ffsHospDiagAt = now; _ffsHospDiagN++;
    try {
      GM_xmlhttpRequest({
        method: "POST",
        url: "https://tornwar.com/api/debug/client-log",
        headers: { "Content-Type": "application/json" },
        data: JSON.stringify({ tag: "ffs-hosp-zero", data: payload }),
        onload: function(){}, onerror: function(){},
      });
    } catch (_) {}
  }
  ...
  function ffs_recordMemberTravel(member) {
    ...
    if (state === "Hospital" || state === "Jail") {
      ...
      // beta: note the moment they became hittable, but only on the EDGE — a
  ...
  function ffs_paintTravelCountdowns() {
    ...
    rows.forEach(row => {
      ...
      if (until) {
        ...
        if (hospUntil) {
          // beta: Torn's own cell outranks our cached timer. This is reachable
          ...
          if (remaining <= 0) {
            // beta: report what Torn's cell actually holds while we are stuck
            // at zero. The positive word list above is a guess at Torn's
            // wording for a released member and this is how it stops being one.
            ffs_hospDiag({
              uid, hospUntil, sinceZero: -Math.round(hospUntil - ffs_nowSecFloat()),
              cellClass: String(statusEl.className || '').slice(0, 120),
              cellAria: (statusEl.getAttribute('aria-label') || '').slice(0, 80),
              cellText: (statusEl.textContent || '').slice(0, 80),
  ...
  function ffs_rowGroup(row, pid) {
    ...
    // beta: just out of hospital, and still hittable. Checked AFTER travel and
  ...
  function ffs_applyWarSort(rowList) {
    ...
    // beta: off the war page, leave the list in Torn's order unless the reader
    ...
    for (const [parent, rows] of groups) {
      ...
      metas.sort((A, B) => {
        ...
        if (warMode) {
          ...
          // beta: freshest release leads — the newest opening is the one nobody
          // else has taken yet.
