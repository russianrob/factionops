// ==UserScript==
// @name         OC Item Worth & Totals (API Key)
// @namespace    RussianRob
// @version      1.0.0
// @description  Shows the real market value of completed-OC reward items and a per-OC "Items total", pulling prices straight from Torn's item market with YOUR own API key. No external server — talks only to api.torn.com. Works in Torn PDA.
// @author       RussianRob
...
// @downloadURL  https://tornwar.com/scripts/oc-item-worth-keyed.user.js
// @updateURL    https://tornwar.com/scripts/oc-item-worth-keyed.meta.js
// ==/UserScript==

// Key-based sibling of OC Item Worth & Totals: instead of fetching prices from
// tornwar.com, it asks for the user's own Torn API key and reads the live
// item-market lowest listing directly from Torn (/v2/market/{id}/itemmarket).
// The key lives only in this script's GM storage and is sent only to
// api.torn.com. Same reward-icon scan + per-OC total as the server version.
(function () {
  "use strict";
