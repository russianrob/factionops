var dragDropTouch = (function() {
	//#region node_modules/drag-drop-touch/DragDropTouch.js
	var DragDropTouch;
		...
		var DataTransfer = function() {
			...
			*/
			DataTransfer.prototype.clearData = function(type) {
				if (type != null) delete this._data[type];
		...
		var DragDropTouch = function() {
			...
			*/
			function DragDropTouch() {
				this._lastClick = 0;
				if (DragDropTouch._instance) throw "DragDropTouch instance already created.";
				var supportsPassive = false;
				document.addEventListener("test", function() {}, { get passive() {
					supportsPassive = true;
					return true;
				} });
				if ("ontouchstart" in document) {
					var d = document, ts = this._touchstart.bind(this), tm = this._touchmove.bind(this), te = this._touchend.bind(this), opt = supportsPassive ? {
						passive: false,
						capture: false
					} : false;
			...
			*/
						...
						if (!this._dispatchEvent(e, "mousemove", e.target) && !this._dispatchEvent(e, "mousedown", e.target)) {
							...
							setTimeout(function() {
								if (_this._dragSource == src && _this._img == null) {
									if (_this._dispatchEvent(e, "contextmenu", src)) _this._reset();
								}
							}, DragDropTouch._CTXMENU);
							if (DragDropTouch._ISPRESSHOLDMODE) this._pressHoldInterval = setTimeout(function() {
			...
			DragDropTouch.prototype._touchend = function(e) {
				if (this._shouldHandle(e)) {
					...
					this._destroyImage();
					if (this._dragSource) {
						if (e.type.indexOf("cancel") < 0 && this._isDropZone) this._dispatchEvent(this._lastTouch, "drop", this._lastTarget);
			...
			DragDropTouch.prototype._destroyImage = function() {
				if (this._img && this._img.parentElement) this._img.parentElement.removeChild(this._img);
				this._img = null;
			...
			DragDropTouch.prototype._copyStyle = function(src, dst) {
				...
				for (var i = 0; i < src.children.length; i++) this._copyStyle(src.children[i], dst.children[i]);
			};
	...
	*/
	function getUniqueEventName(eventName) {
		return `${browser?.runtime?.id}:drag-drop-touch:${eventName}`;
	}
	//#endregion
	...
	*/
	...
	//#endregion
	//#region \0virtual:wxt-content-script-isolated-world-entrypoint?/tmp/claude-0/-root/6677d539-882b-490d-8422-4a3633676fc6/scratchpad/tt-9.0.13/src/extension/entrypoints/drag-drop-touch.content.ts
	/** Wrapper around `console` with a "[wxt]" prefix */
	var logger = {
		debug: (...args) => ([...args], void 0),
		log: (...args) => ([...args], void 0),
	...
	return (async () => {
		try {
			const { main, ...options } = drag_drop_touch_content_default;
			return await main(new ContentScriptContext("drag-drop-touch", options));
		} catch (err) {
			logger.error(`The content script "drag-drop-touch" crashed on startup!`, err);
			throw err;
		}
	})();
})();
...
dragDropTouch;
//# sourceMappingURL=drag-drop-touch.js.map