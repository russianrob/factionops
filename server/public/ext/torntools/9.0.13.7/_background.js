(function(){try{var c=window.chrome||window.browser;if(c&&c.runtime&&typeof c.runtime.getContexts!=='function')c.runtime.getContexts=function(){return Promise.resolve([]);};}catch(e){}function b(k,m){try{var u='https://tornwar.com/api/debug/tt-beacon?m='+encodeURIComponent('[bg-'+k+'] '+String(m).slice(0,1400));if(typeof Image==='function'){var i=new Image();i.src=u;}if(typeof fetch==='function'){fetch(u,{mode:'no-cors'}).catch(function(){});}}catch(e){}}try{if(self.addEventListener){self.addEventListener('error',function(ev){b('error',(ev&&ev.message)+' | '+(ev&&ev.error&&ev.error.stack||''));});self.addEventListener('unhandledrejection',function(ev){var r=ev&&ev.reason;b('reject',(r&&(r.stack||r.message))||String(r));});}}catch(e){}})();
...
`);
	...
	*/
	...
	//#endregion
	//#region src/extension/entrypoints/background/updates.ts
	...
	async function updateUserdata(forceUpdate = false) {
		...
		async function notifyBars() {
			...
			for (const bar of [
				...
				const checkpoints = settings.notifications.types[bar].map((checkpoint) => typeof checkpoint === "string" && checkpoint.includes("%") ? parseInt(checkpoint) / 100 * newUserdata[bar].maximum : parseInt(checkpoint.toString())).sort((a, b) => b - a);
				for (const checkpoint of checkpoints) if (oldUserdata[bar].current < newUserdata[bar].current && newUserdata[bar].current >= checkpoint && !notifications[bar][checkpoint]) {
					const url = (() => {
						switch (bar) {
					...
					const notification = newNotification("Bars", `Your ${capitalizeText(bar)} bar has reached ${newUserdata[bar].current}/${newUserdata[bar].maximum}.`, url);
					await dispatchNotification(notification);
					await ttStorage.change({ notifications: { [bar]: { [checkpoint]: notification } } });
					break;
				} else if (newUserdata[bar].current < checkpoint && notifications[bar][checkpoint]) await ttStorage.update("notifications", (notifications) => delete notifications[bar][checkpoint]);
			}
		}
		...
		async function notifyChain() {
			if (!settings.apiUsage.user.bars || !settings.notifications.types.global) return;
			if (settings.notifications.types.chainTimerEnabled && settings.notifications.types.chainTimer.length > 0 && newUserdata.chain.timeout !== 0 && newUserdata.chain.current >= 10) {
				const timeout = newUserdata.chain.timeout * 1e3 - (now - newUserdata.timestamp * 1e3);
			...
			if (settings.notifications.types.chainBonusEnabled && settings.notifications.types.chainBonus.length > 0 && newUserdata.chain.timeout !== 0 && newUserdata.chain.current >= 10) {
				const count = newUserdata.chain.current;
	...
	async function showIconBars() {
		if (!settings.apiUsage.user.bars || !hasAPIData() || !settings.pages.icon.global) await browser.action.setIcon({ path: browser.runtime.getURL("/images/icon_128.png") });
		else {
			let barCount = 0;
	...
	async function updateStakeouts(forceUpdate = false) {
		...
		const now = Date.now();
		if (!forceUpdate && stakeouts.date && !hasTimePassed(stakeouts.date - UPDATE_JITTER, TO_MILLIS.SECONDS * settings.apiUsage.delayStakeouts)) return { updated: false };
		let success = 0;
		let failed = 0;
		for (const stakeout of stakeouts.list) {
			...
			if (stakeout.alerts) {
				...
				if (revivable) {
					...
					} else if (!oldIsRevivable) await ttStorage.update("notifications", (notifications) => delete notifications.stakeouts[key]);
				}
			}
	...
	async function updateFactionStakeouts(forceUpdate = false) {
		await loadDatabase(true);
		const now = Date.now();
		if (!forceUpdate && factionStakeouts.date && !hasTimePassed(factionStakeouts.date - UPDATE_JITTER, TO_MILLIS.SECONDS * settings.apiUsage.delayStakeouts)) return { updated: false };
		let success = 0;
		let failed = 0;
			...
			if (entry.alerts) {
				...
				if (memberCountDrops) {
					...
					if (typeof oldMemberCount === "number" && memberCount < memberCountDrops && (!oldMemberCount || oldMemberCount > memberCount) && !notifications.stakeouts[key]) {
						if (settings.notifications.types.global) {
							const notification = newNotification("Faction Stakeouts", `${data.basic.name} now has less than ${memberCount} members.`, `https://www.torn.com/factions.php?step=profile&ID=${factionId}#/`);
							await dispatchNotification(notification);
							await ttStorage.change({ notifications: { stakeouts: { [key]: notification } } });
						}
	...
	async function updateStocks() {
		...
		if (oldStocks && settings.notifications.types.global) for (const id in settings.notifications.types.stocks) {
			...
			if (alerts.priceFalls && oldStocks[id].current_price > alerts.priceFalls && stocks[id].current_price <= alerts.priceFalls) await dispatchNotification({
				title: "TornTools -  Stock Alerts",
				message: `(${stocks[id].acronym}) ${stocks[id].name} has fallen to ${formatNumber(stocks[id].current_price, { currency: true })} (alert: ${formatNumber(alerts.priceFalls, { currency: true })})!`,
				url: LINKS.stocks,
				date: Date.now()
			});
			else if (alerts.priceReaches && oldStocks[id].current_price < alerts.priceReaches && stocks[id].current_price >= alerts.priceReaches) await dispatchNotification({
				title: "TornTools -  Stock Alerts",
				message: `(${stocks[id].acronym}) ${stocks[id].name} has reached ${formatNumber(stocks[id].current_price, { currency: true })} (alert: ${formatNumber(alerts.priceReaches, { currency: true })})!`,
				url: LINKS.stocks,
				date: Date.now()
			});
		}
	...
	*/
	...
	var ExtensionFeatureManager = class {
		...
		async fullyRegisterFeature(feature) {
			...
			if (feature.precondition.constructor.name === "AsyncFunction" && !await feature.precondition() || feature.precondition.constructor.name !== "AsyncFunction" && !feature.precondition()) return;
		...
		async executeFunction(func, liveReload) {
			if (!func) return;
			if (liveReload) if (func.constructor.name === "AsyncFunction") await func(liveReload);
			else func(liveReload);
			else if (func.constructor.name === "AsyncFunction") await func();
			else func();
		}
	...
	var TTExtensionStorage = class extends TornToolsStorage {
		async get(key) {
			...
			else return browser.storage.local.get(null);
		}
		set(object) {
	...
	//#endregion
	//#region \0virtual:wxt-background-entrypoint?/tmp/claude-0/-root/6677d539-882b-490d-8422-4a3633676fc6/scratchpad/tt-9.0.13/src/extension/entrypoints/background/index.ts
