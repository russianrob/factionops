...
// chain) are nobody's doing. Warlord is different in kind — it is a weapon
// bonus the member went and got — and the faction asked for it to count.
//
...
test("by default a warlord bonus now counts toward the score", () => {
  ...
  assert.ok(boosted["2"].fairScoreSum > plain["1"].fairScoreSum,
    "the warlord hit scored no more than the plain one");
  assert.equal(boosted["2"].fairScoreSum, 20);
  assert.equal(plain["1"].fairScoreSum, 10);
});

test("and stripping it makes those two equal again, which was the old point", () => {
  const plain   = score([hit("1", 20, { warlord_bonus: 1 })], { includeWarlord: false });
  const boosted = score([hit("2", 40, { warlord_bonus: 2 })], { includeWarlord: false });
  assert.equal(boosted["2"].fairScoreSum, plain["1"].fairScoreSum);
});

test("the old behaviour is still available, and strips it exactly", () => {
  // respect 20 with war 2 and warlord 2 -> 20 / (2*2) = 5.
  const off = score([hit("1", 20, { warlord_bonus: 2 })], { includeWarlord: false });
  assert.equal(Math.round(off["1"].fairScoreSum * 1000) / 1000, 5);
});

test("with it counted, only war and chain are divided out", () => {
  // respect 20 with war 2 and warlord 2 -> 20 / 2 = 10.
  const on = score([hit("1", 20, { warlord_bonus: 2 })], {});
...
test("a non-war hit follows the same rule", () => {
  ...
  const on = score([nonWarHit("1", 20, { warlord_bonus: 2, war: 1 })], {});
  const off = score([nonWarHit("1", 20, { warlord_bonus: 2, war: 1 })], { includeWarlord: false });
