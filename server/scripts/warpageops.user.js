...
(function () {
    ...
    const CONFIG = {
        ...
        ENEMY_ATTACK_NOTIF: GM_getValue('warpageops_enemy_attack_notif', false),
        KEEP_ALIVE: GM_getValue('warpageops_keep_alive', false),
        ...
        // for those targets.
        SHARE_BSP: GM_getValue('warpageops_share_bsp', false),
    ...
    if (IS_PDA && !GM_getValue('warpageops_apikey', '')) {
        GM_setValue('warpageops_apikey', PDA_API_KEY);
    }
    ...
    function setConfig(key, value) {
        CONFIG[key] = value;
        const gmKeys = {
            SERVER_URL: 'warpageops_server',
            API_KEY: 'warpageops_apikey',
            THEME: 'warpageops_theme',
            AUTO_SORT: 'warpageops_autosort',
            CHAIN_ALERT: 'warpageops_chain_alert',
            CHAIN_ALERT_THRESHOLD: 'warpageops_chain_alert_threshold',
            PDA_NOTIFICATIONS: 'warpageops_pda_notif',
            ENEMY_ATTACK_NOTIF: 'warpageops_enemy_attack_notif',
            KEEP_ALIVE: 'warpageops_keep_alive',
            SHARE_BSP: 'warpageops_share_bsp',
        };
        if (gmKeys[key]) {
...
`;
    ...
    const SORT_MODE_KEY = 'warpageops_sortmode';
    let _sortMode = (() => {
    ...
    const state = {
        connected: false,
        connecting: false,
        jwtToken: GM_getValue('warpageops_jwt', ''),
        myPlayerId: null,
        myPlayerName: null,
        myFactionId: null,
        myFactionName: null,
    ...
    async function refreshFlightsForTravelers() {
        ...
        const apiKey = GM_getValue('warpageops_api_key', '') || CONFIG.API_KEY;
            ...
            httpRequest({
                ...
                onload(res) {
                    ...
                    if (res.status >= 200 && res.status < 300 && body && body.token) {
                        ...
                        GM_setValue('warpageops_jwt', body.token);
                        if (body.player) {
    ...
    function initWarOverlay() {
        ...
        function applyStatsFilter() {
            ...
            try {
                GM_setValue('warpageops_stats_filter_min', _statsFilterMin || 0);
                GM_setValue('warpageops_stats_filter_max', _statsFilterMax || 0);
            } catch (_) {}
        ...
        if (hideOnlineEl) {
            ...
            hideOnlineEl.addEventListener('change', () => {
                ...
                try { GM_setValue('warpageops_hide_online', _hideOnline); } catch (_) {}
                renderOverlay();
            });
        }
        ...
        if (hideOfflineEl) {
            ...
            hideOfflineEl.addEventListener('change', () => {
                ...
                try { GM_setValue('warpageops_hide_offline', _hideOffline); } catch (_) {}
                renderOverlay();
            });
        }
    ...
    try {
        const m = Number(GM_getValue('warpageops_stats_filter_min', 0));
        if (Number.isFinite(m) && m > 0) _statsFilterMin = m;
        const x = Number(GM_getValue('warpageops_stats_filter_max', 0));
        if (Number.isFinite(x) && x > 0) _statsFilterMax = x;
        _hideOnline = !!GM_getValue('warpageops_hide_online', false);
        _hideOffline = !!GM_getValue('warpageops_hide_offline', false);
    } catch (_) {}
    ...
    function setupTabCoordination() {
        try {
            broadcastChannel = new BroadcastChannel('warpageops_sync');
    ...
    // ═══════════════════════════════════════════════════════════════════════
    let _htmModalState = {
        activeTab: GM_getValue('warpageops_htm_tab', 'activity'),
        activityData: null,
        activityFetchedAt: 0,
    };
    ...
    async function loadHtmPayouts() {
        ...
        if (!pane.dataset.initialised) {
            ...
            pane.querySelector('#wb-htm-payouts-mode').addEventListener('change', async (e) => {
                ...
                try { GM_setValue('warpageops_payouts_mode', e.target.value); } catch (_) {}
    ...
    let _payoutsModalState = {
        mode: GM_getValue('warpageops_payouts_mode', 'dynamic'),
        selectedWarId: null,
        data: null,
    };
    ...
    async function renderCombinedHeatmapPanel(ourId, ourName, enemyId, enemyName) {
        ...
        const savedPos = GM_getValue('warpageops_heatmap_pos', null);
        if (savedPos) {
            try {
        ...
        document.addEventListener('mouseup', () => {
            if (isDragging) {
                isDragging = false;
                GM_setValue('warpageops_heatmap_pos', JSON.stringify({
