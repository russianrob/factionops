...
const mutants = [
  ...
  ["the rungs go back to flat, levelling the build instead of holding it",
    "    return Math.min(L * (shares[k] || 0) / maxShare, target);",
    "    return Math.min(L, target);", [U]],
  ["a build is ignored by the ladder entirely",
    "    if (!shares || !(maxShare > 0)) return Math.min(L, target);",
    "    return Math.min(L, target);", [U]],
  ["the scale uses the wrong stat's share",
    "L * (shares[k] || 0) / maxShare", "L * maxShare / maxShare", [U]],
  ["the target no longer caps the rung, so a stat overshoots its goal",
    "    return Math.min(L * (shares[k] || 0) / maxShare, target);",
    "    return L * (shares[k] || 0) / maxShare;", [U]],
  ["the card marker ignores the planner again and contradicts the verdict",
