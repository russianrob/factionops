...
const mutants = [
  ...
  ["a page-detected book is a toggle again, so tapping it resets the countdown",
    "      var owned = p && ((state.booksAuto || {})[k] || exact);", "      var owned = false;", [E]],
  ["nothing is tappable, so a book Torn cannot show can never be recorded",
    "      var owned = p && ((state.booksAuto || {})[k] || exact);", "      var owned = true;", [E]],
  ["the countdown rounds to whole days again, disagreeing with Torn's own icon",
    '          : Math.floor(ms / 86400000) + "d " + Math.floor((ms % 86400000) / 3600000) + "h left";',
    '          : Math.ceil(ms / 86400000) + "d left";', [E]],
  ["a book with days left is reported in hours",
    "        ms < 172800000", "        false", [E]],
  ["the other three books are offered even though the page named the one",
    '    var rows = !live.length', '    var pageKnows = false; var rows = !live.length', [E]],
  ["a hand-recorded book stops offering the others, leaving no way to change it",
    "      return (state.booksAuto || {})[k] || (state.booksExact || {})[k];", "      return true;", [E]],
  ["the cap threshold is hardcoded rather than derived from the two constants",
    "    return BOOK_PCT > 0 ? Math.round(BOOK_CAP / BOOK_PCT) : 0;", "    return 200000000;", [U]],
  ["the cap threshold is never stated",
    '      " days \\u2014 so the cap bites at <b>" + fmt(bookCapAt()) + "</b>, and above that " +\n      "every book is worth a flat " + fmt(BOOK_CAP) + " however big the stat gets.',
    '      " days.', [E]],
