...
const mutants = [
  ...
  ["the name swallows the effect sentence and matches no book",
    "      var name = String(label).replace(BOOK_LABEL_RE, \"\").split(BOOK_DASH_RE)[0].trim();",
    "      var name = String(label).replace(BOOK_LABEL_RE, \"\").trim();", [U]],
  ...
  ["a non-stat book is credited to a stat anyway",
    "      return { found: true, name: name, k: k };", '      return { found: true, name: name, k: k || "str" };', [U, E]],
